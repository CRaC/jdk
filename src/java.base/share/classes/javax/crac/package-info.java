 ...
 * Coordination allows restored instances to behave differently according to a dynamic execution environment.
 * <p>
 * Coordination requires Java application not to capture parts of execution environment which may change after restore.
 * The Java implementation provides best-effort checks that no such parts are captured by the application.
 * Checkpoint notification allows the application to prepare for checkpoint and satisfy checks.
 * After the restore, corresponding notification allows the application to re-initialize in the new environment.
 * <p>
